...
`implementations/languages.json` is the 50-language target index. `implementations/coverage-summary.json` is the current implementation ledger, and `implementations/verified-cells.json` is the proof list for cells that have passing local test commands. Current coverage is partial: 50,000 planned cells, 11 verified cells, and 49,989 cells still planned.
